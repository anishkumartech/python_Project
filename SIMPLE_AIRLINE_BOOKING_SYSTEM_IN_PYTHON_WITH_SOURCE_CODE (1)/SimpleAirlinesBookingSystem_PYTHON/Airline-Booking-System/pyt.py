...
def fun8():
    ...
    w2=ttk.Combobox(root2,height=5,width=15,values=["BusinessClass","Economic"])
    w2.grid(row=1,column=1)
    Label(root2,text="select boarding").grid(row=2,column=0)
    w3=ttk.Combobox(root2,height=5,width=15,values=["New York","Chicago","Dallas","San Francisco"])
    ...
    def fun2():
        ...
        con.commit()
        if d=='' or b=='' or c=='':
             tkinter.messagebox.showerror("Oops","You can't Enter the leave any field empty")
        else:     
             if b=="economic":
...
def fun9():
    ...
    w1=ttk.Combobox(root4,height=5,width=15,values=["New York","Chicago","Dallas","San Francisco"])
    w1.grid(row=0,column=1)
    Label(root4,text="select destination").grid(row=1,column=0)
    w2=ttk.Combobox(root4,height=5,width=15,values=["New York","Chicago","Dallas","San Francisco"])
    w2.grid(row=1,column=1)
        ...
        if a=='' or b=='' or c=='':
            tkinter.messagebox.showerror("Error","Cant leave any field empty")
             ...
             if a!=b:
                 ...
                 con.commit()
                 e=cur.fetchall()
                 tkinter.messagebox.showinfo("flights availble are",e)
             else:
...
#e1.grid(row=1,column=1)
    w=ttk.Combobox(root,height=5,width=15,values=["Dallas","Washington","New York","San Francisco"])
    w.grid(row=1,column=1)
    Label(root,text='Enter Destination').grid(row=2,column=0)
#e2=Entry(root,width=20,justify='right')
#e2.grid(row=2,column=1)
    w1=ttk.Combobox(root,height=5,width=15,values=["Dallas","Washington","New York","San Francisco"])
...
#e3.grid(row=3,column=1)
    ...
    w2=ttk.Combobox(root,text='BusinessClass',height=5,width=15,values=["BusinessClass","Economic"])
    ...
    w3=ttk.Combobox(root,text="choose day",height=5,width=15,values=["sunday","monday","tuesday","wensday","thursday","friday","saturday"])
    w3.grid(row=5,column=1)
    Label(root,text="choose time of your flight").grid(row=6,column=0)
    w4=ttk.Combobox(root,height=5,width=15,values=["1:00 AM","7:00 AM","1:00 PM","4:00 PM","9:00 PM"])
